...
class ContestsViewModel  @Inject constructor(
    ...
    fun getContest(resource:String,start_dt:LocalDateTime,end_dt:LocalDateTime)=viewModelScope.launch(Dispatchers.IO) {
        contestDetails.postValue(Resource.Loading())
        try {
            if(isInternetAvailable(app)){

                val apiResult = getContestUseCase.execute(resource,start_dt,end_dt)
